...
def generate_historical_data_by_folder_PC(graphs_path, data_path, info_path, n, gamma_min,
                             gamma_max, thres_min, thres_max, prob_inter, epsilon, seed=3344, only_inter=False):
            ...
            if len(parents) != 0:
                for i in range(len(data[node])):
                    ...
                    if abnormal_parent != 0:
                        for i in range(abnormal_parent):
...
# thres_min: minimum threshold for all nodes
# thres_max: maximum threshold for all nodes
def generate_simulation_data_by_folder_PC(graphs_path, info_path, save_data_path, save_info_path, n, epsilon, seed=3344, only_inter=False, num_inters=1):
    # np.random.seed(seed=seed)
    ...
    for json_file_path in graph_files:
        ...
        if only_inter:
            ...
            for node in nodes_list:
                data[node] = []
                if node not in children_list or node in inter_nodes:
                    data[node] = np.random.uniform(low=0, high=1, size=n)
                else:
                    data[node] = np.random.uniform(low=0, high=dict_nodes_thres[node][0], size=n)
...
if __name__ =='__main__':
    ...
    # example: generate data for the whole folder of the graph
    graphs_path = 'graphs'
    info_path = os.path.join('uncertain_0.3', 'data_info')
    n = 500 # sampling points
    epsilon = 0.3
    num_inters = 2
    # data_path = os.path.join('uncertain_'+str(epsilon), 'data')
    # info_path = os.path.join('uncertain_'+str(epsilon), 'data_info')
    save_data_path = os.path.join('uncertain_'+str(epsilon), 'actual_data_'+str(num_inters)+'_inters')
    save_info_path = os.path.join('uncertain_'+str(epsilon), 'data_info_'+str(num_inters)+'_inters')
    generate_simulation_data_by_folder_PC(graphs_path=graphs_path, info_path=info_path, save_data_path=save_data_path, save_info_path=save_info_path,
                                       n=n, epsilon=epsilon, only_inter=True, num_inters=num_inters)
