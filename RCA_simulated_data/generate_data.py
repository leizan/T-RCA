...
def generate_data_with_parametric_intervention(DAG,n, secondInterventionNode, rootStartIntervention=0, rootEndIntervention=0):
    ...
    new_coef = list(np.random.uniform(low=1, high=2, size=2)) # [0]

    intervention_coef = {secondInterventionNode[0]: new_coef[0],
                         secondInterventionNode[1]: new_coef[1]}
    ...
    for i in range(rootStartIntervention, rootEndIntervention):
        for node in orderingNodes[1:]:
            ...
            for par in DAG.predecessors(node):
                ...
                if node in secondInterventionNode:
                    ...
                    data[node].loc[i] = noise[node].loc[i] + coef_dict[(node, node)]*data[node].loc[i-1] + intervention_coef[node] * data[par].loc[i-1]
