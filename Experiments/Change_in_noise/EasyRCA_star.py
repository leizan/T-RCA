...
for mechanisme in list_mechanisme:
    for scenario in list_scenarios:
            ...
            for sig_level in list_sig_level:  #np.arange(0.01, 0.2, 0.04).tolist():
                ...
                for data_path in tqdm(data_files):
                    ...
                    for num_inter in list_num_inters:
                        ...
                        # if True:
                       #      erca = EasyRCA(summary_graph=g, anomalous_nodes=anomalous_nodes, anomalies_start_time=anomalies_start_time,
                       # anomaly_length=anomaly_length, gamma_max=gamma_max, sig_threshold=sig_level)
                            erca = EasyRCA(summary_graph=g, anomalous_nodes=anomaly_nodes, anomalies_start_time=anomalies_start_time,
                                anomaly_length=anomaly_length, gamma_max=gamma_max, sig_threshold=sig_level)
        ...
        simple_res_path = os.path.join('..', '..', 'Results_sim_20000', mechanisme, scenario, 'EasyRCA_star.json')
        with open(simple_res_path, 'w') as json_file:
            json.dump(simple_final_res, json_file)

        complete_res_path = os.path.join('..', '..', 'Results_com_20000', mechanisme, scenario, 'EasyRCA_star.json')
