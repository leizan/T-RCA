...
def dict_to_graph(graph_nx, inter_nodes, str_to_node):
    ...
    for node in graph_nx.nodes:
        graph.add_node(str_to_node[name_to_CIRCA[node]])
    for edge in graph_nx.edges:
        graph.add_edge(str_to_node[name_to_CIRCA[edge[0]]], str_to_node[name_to_CIRCA[edge[1]]])
    return graph
...
for mechanisme in list_mechanisme:
    for scenario in list_scenarios:
            ...
            for sig_level in list_sig_level:  #np.arange(0.01, 0.2, 0.04).tolist():
                ...
                for data_path in tqdm(data_files):
                    ...
                    for num_inter in list_num_inters:
                        ...
                        true_inter_graph = dict_to_graph(graph_nx=causal_graph, inter_nodes=[], str_to_node=str_to_node)
        ...
        simple_res_path = os.path.join('..', '..', 'Results_sim_20000', mechanisme, scenario, 'CIRCA.json')
        with open(simple_res_path, 'w') as json_file:
            json.dump(simple_final_res, json_file)

        complete_res_path = os.path.join('..', '..', 'Results_com_20000', mechanisme, scenario, 'CIRCA.json')
