...
list_sampling_number = [10,20,50,100,200,500,1000,2000] #[10, 100, 500, 1000, 2000]
list_num_inters = [2]
...
for mechanisme in list_mechanisme:
    for scenario in list_scenarios:
        ...
        # simple_res_path = os.path.join('..', '..', 'Results_sim_20000', mechanisme, scenario, 'EasyRCA.json')
        # with open(simple_res_path, 'r') as json_file:
        #     simple_final_res = json.load(json_file)
        #
        # complete_res_path = os.path.join('..', '..', 'Results_com_20000', mechanisme, scenario, 'EasyRCA.json')
                ...
                for data_path in tqdm(data_files):
                    ...
                    for num_inter in list_num_inters:
                       ...
                       # anomaly_length=anomaly_length, gamma_max=gamma_max, sig_threshold=sig_level)
                        ...
                        erca = EasyRCA(summary_graph=true_inter_graph, anomalous_nodes=anomalous_node, anomalies_start_time=anomalies_start_time,
                                    anomaly_length=anomaly_length, gamma_max=gamma_max, sig_threshold=sig_level)
                        erca.run(new_actual_data)    # erca.run(actual_data)
                        # print('The size of actual data')
                        # print(new_actual_data.values.shape[0])
                       ...
                       # # anomaly_length=anomaly_length, gamma_max=gamma_max, sig_threshold=sig_level)
                       #      # print('nodes in the graph')
                       #      erca = EasyRCA(summary_graph=true_inter_graph, anomalous_nodes=actual_data.columns, anomalies_start_time=anomalies_start_time,
                       # anomaly_length=anomaly_length, gamma_max=gamma_max, sig_threshold=sig_level)
                       #      erca.run(new_actual_data)    # erca.run(actual_data)
                       #      pred_root_causes_dict = erca.root_causes
                       #      pred_root_causes = []
