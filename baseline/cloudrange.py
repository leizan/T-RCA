...
from baseline.estimation import FisherZ
...
def find_transition_matrix(g, df, backward_step=0.1):
    ...
    # Self step
    adj = nx.to_numpy_array(g) # adj = nx.to_numpy_matrix(g)
...
def cloud_ranger(data, anomalous_nodes, anomalies_start_time=None, anomaly_length=200, sig_threshold=0.05):
    ...
    output = pc(anomalous_data.values, sig_threshold, "fisherz", verbose=False)
