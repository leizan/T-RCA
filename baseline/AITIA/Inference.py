...
class Inference:
    ...
    def __init__(self, data, pb) -> None:
        self.source = data # pd.read_csv(file_path)
    ...
    def calculate_average_epsilons(self, target_file=None) -> None:
        """
        ...
        for effect in self.prima_facie:
            for cause in self.generate_iterator(self.prima_facie[effect], desc = "Calculating Epsilon values"):
                epsilon_avg = self.get_epsilon_average(effect, cause)
                dict_epsilon[(cause, effect)] = epsilon_avg
